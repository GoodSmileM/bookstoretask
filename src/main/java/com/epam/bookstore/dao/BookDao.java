...
public interface BookDao extends JpaRepository<Book, Long>, Serializable {
    ...
    Integer countSoldByCategory(String category);

    @Query(value = "select sum(sold) from book_info where category=?2 and (id like ?1 or author like ?1 or title like ?1)", nativeQuery = true)
